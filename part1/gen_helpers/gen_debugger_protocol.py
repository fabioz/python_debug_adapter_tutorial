...
def update_class_to_generate_register_dec(class_to_generate):
    # Default
    ...
    if enum_type and len(enum_type) == 1 and next(iter(enum_type)) in ("request",):
...
def update_class_to_generate_refs(class_to_generate):
    properties = class_to_generate['properties']
    class_to_generate['refs'] = '    __refs__ = %r' % set(key for (key, val) in properties.items() if val['type'].__class__ == Ref)
...
def gen_debugger_protocol():
    ...
    for class_to_generate in classes_to_generate.values():
        ...
        update_class_to_generate_register_dec(class_to_generate)
...
'''
    ...
    contents.append('from .base_schema import BaseSchema, register, register_request')
    contents.append('')
    for class_to_generate in classes_to_generate.values():
